...
TfLiteStatus MLModel::predict()
{
    // The predict function now only runs the interpreter and returns the status.
    // The caller is responsible for fetching and de-quantizing the output data.
    TfLiteStatus invoke_status = _interpreter->Invoke();

    return invoke_status;
}

void* MLModel::get_output_data()
{
    if (_output_tensor == NULL) {
        return NULL;
    }
    // Returns a pointer to the start of the 4-element output buffer.
